import {
  Flex, IconButton, VStack, Text, Image,
...
export default function UserConversation({ convo }:Props) {
  ...
  const lastMessage = useAppSelector((state) => state
    .messagesSlice
    .conversationMessages[convo.conversationId].messages.at(-1));
  const handleClick = () => {
  ...
  const isActiveConvo = useAppSelector(
    (state) => state.convoSlice.activeConvo?.conversationId === convo.conversationId,
  );
  return (
    <Flex
      maxW="100%"
